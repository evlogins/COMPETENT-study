#!/usr/bin/env python3
# ============================================================
# COMPETENT STUDY — COMPREHENSIVE ANALYSIS (v3 - Local)
# Addresses all 10 significant statistical concerns
# ============================================================
# Adapted from Google Colab version for local / Claude Code use.
#
# CHANGELOG from v2:
#  1. NNH now includes 95% CI (Newcombe method)
#  2. Skewed variables reported as median (IQR), not mean±SD
...
#  8. Hypotension–vasopressor disconnect discussed
...
DATA_PATH = os.path.join(REPO_ROOT, 'data', 'VIMAMC_MH.xlsx')
...
# LOAD DATA
# ────────────────────────────────────────────────────────────
df = pd.read_excel(DATA_PATH, sheet_name='Endpunkttabelle')

# Narkose coding: 1 = Sevoflurane (n=32), 0 = Etomidate (n=25)
...
# LVF coding: 1=<30%, 2=30-44%, 3=45-54%, 4=≥55%
df['LVF_reduced'] = (df['LVF'] <= 2).astype(int)  # LVEF ≤44%
...
print("="*70)
print("""
The following column names contain OCR/data-entry artifacts:
  - 'TO_MAP'  and 'TO_Hf'  → likely T0_MAP and T0_Hf (zero vs letter O)
  - 'T!_MAP'  and 'T!_Hf'  → likely T1_MAP and T1_Hf (one vs exclamation)
...
def compare_continuous_median(var, label):
    ...
    print(f"  {label}:")
    print(f"    Sevoflurane: {s_med:.1f} ({s_q1:.1f}–{s_q3:.1f})  [n={len(s)}]")
    print(f"    Etomidate:   {e_med:.1f} ({e_q1:.1f}–{e_q3:.1f})  [n={len(e)}]")
    print(f"    P {fmt_p(p)}")
    return p

def compare_continuous_mean(var, label):
    """Report mean ± SD for approximately normal variables."""
    s = sevo[var].dropna()
    e = etom[var].dropna()
    stat, p = mannwhitneyu(s, e, alternative='two-sided')
    print(f"  {label}:")
    print(f"    Sevoflurane: {s.mean():.1f} ± {s.std():.1f}  [n={len(s)}]")
    print(f"    Etomidate:   {e.mean():.1f} ± {e.std():.1f}  [n={len(e)}]")
    print(f"    P {fmt_p(p)}")
    return p
...
print("\n--- Demographics (mean ± SD; Mann-Whitney U) ---")
compare_continuous_mean('Alter', 'Age (years)')
compare_continuous_mean('BMI', 'BMI (kg/m²)')
compare_continuous_mean('Gewicht', 'Weight (kg)')
compare_continuous_mean('Größe', 'Height (cm)')

print("\n--- Comorbidities [n (%); Fisher exact] ---")
compare_binary('LVF_reduced', 'LVEF ≤44%')
...
p_remi = compare_continuous_mean('Remifentanil', 'Remifentanil (µg)')

# ────────────────────────────────────────────────────────────
# PRIMARY OUTCOME — CONCERN 2: MEDIAN (IQR) FOR SKEWED DATA
# ────────────────────────────────────────────────────────────
print("\n" + "="*70)
...
p_nor_dose = compare_continuous_median('Noradrenalin', 'Noradrenaline (µg)')
...
# ────────────────────────────────────────────────────────────
# REGRESSION MODELS — CONCERN 5: LVF AS COVARIATE
# ────────────────────────────────────────────────────────────
print("\n" + "="*70)
...
for name, formula in model_specs:
    ...
    print(f"\n  {name}")
    print(f"    OR = {or_val:.2f} (95% CI: {ci[0]:.2f}–{ci[1]:.2f}), P {fmt_p(p)}")
    print(f"    Pseudo R² = {m.prsquared:.3f}")
...
print("  (All VIF < 5 → no concerning multicollinearity)")
...
print("(⚠ Limited power — interpret with extreme caution)")
print("="*70)

for subvar, sublabel, cats in [
    ('LVF_reduced', 'LVEF', [(1,'Reduced ≤44%'), (0,'Preserved >44%')]),
    ('Age_over70',  'Age',  [(1,'≥70 years'),    (0,'<70 years')])
]:
    print(f"\n--- By {sublabel} ---")
...
# ────────────────────────────────────────────────────────────
# CONCERN 8: HYPOTENSION ANALYSIS
# ────────────────────────────────────────────────────────────
print("\n" + "="*70)
...
print("""
  Family 1 (Primary): Cafedrine-theodrenaline use
    → P < 0.001 (remains significant under any correction)

  Family 2 (Secondary vasopressors): Noradrenaline, any vasopressor
    → Bonferroni α = 0.05/2 = 0.025

  Family 3 (Hemodynamics): 5 timepoints × 4 parameters → descriptive
  Family 4 (Hypotension): 5 timepoints → exploratory
  Subgroup and propensity analyses → hypothesis-generating
""")
...
print(f"""
  Unadjusted:    OR = {r_unadj[1]:.2f} (95% CI: {r_unadj[2]:.2f}–{r_unadj[3]:.2f})
  + Remifentanil: OR = {r_remi[1]:.2f} (95% CI: {r_remi[2]:.2f}–{r_remi[3]:.2f})  Δ = {pct_change:+.1f}%
  Fullest model:  OR = {r_full[1]:.2f} (95% CI: {r_full[2]:.2f}–{r_full[3]:.2f})  Δ = {pct_full:+.1f}%

The effect is robust across all adjustment strategies.
""")
...
xerr = [np.array(ors) - np.array(ci_lo), np.array(ci_hi) - np.array(ors)]
...
ax.set_title('Odds of Vasopressor Requirement: Etomidate vs Sevoflurane\n'
             '(Outcome: Cafedrine-Theodrenaline Administration)', fontweight='bold')
ax.set_xscale('log')
ax.set_xlim([0.4, 80])
ax.set_xticks([0.5, 1, 2, 5, 10, 20, 50])
ax.get_xaxis().set_major_formatter(plt.ScalarFormatter())
for i, (o, lo, hi) in enumerate(zip(ors, ci_lo, ci_hi)):
    ax.text(68, i, f'{o:.1f} ({lo:.1f}–{hi:.1f})', va='center', fontsize=10,
            fontfamily='monospace')
ax.text(0.45, -1.0, '← Favours Etomidate\n   (lower vasopressor need)',
        fontsize=9, ha='left', va='top', style='italic', color='#555')
ax.text(65, -1.0, 'Favours Sevoflurane →\n(lower vasopressor need)',
        fontsize=9, ha='right', va='top', style='italic', color='#555')
ax.set_ylim([-1.3, len(disp)-0.5])
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, 'Fig1_ForestPlot.png'), dpi=300, bbox_inches='tight')
plt.close()
print("Saved: Fig1_ForestPlot.png")

# ── FIGURE 2: VASOPRESSOR INCIDENCE ──
...
p_vals_bar = []
for v in vars_bin:
    sy, sn_ = int(sevo[v].sum()), len(sevo) - int(sevo[v].sum())
    ey, en_ = int(etom[v].sum()), len(etom) - int(etom[v].sum())
    _, p = fisher_exact([[sy, sn_], [ey, en_]])
    p_vals_bar.append(p)
...
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, 'Fig2_Vasopressors.png'), dpi=300, bbox_inches='tight')
plt.close()
...
def plot_hemo(ax, cols, ylabel, title, threshold=None, thresh_label=None):
    for grp, color, marker in [('Sevoflurane','steelblue','o'),
        ...
        ax.errorbar(timepoints, means, yerr=sems, label=grp,
                    marker=marker, capsize=4, color=color, linewidth=2, markersize=7)
    if threshold:
        ax.axhline(y=threshold, color='gray', linestyle='--', alpha=0.6, label=thresh_label)
    for i, c in enumerate(cols):
        _, p = mannwhitneyu(sevo[c].dropna(), etom[c].dropna())
        if p < 0.05:
            ymax = max(sevo[c].mean(), etom[c].mean()) + 8
            ax.text(i, ymax, '*', ha='center', fontsize=14, color='red', fontweight='bold')
    ax.set_ylabel(ylabel)
    ax.set_title(title, fontweight='bold')
    ax.legend(fontsize=9, loc='upper right')
    ax.set_xlabel('Timepoint')

plot_hemo(axes[0,0], sys_cols, 'SBP (mmHg)', 'A) Systolic Blood Pressure', 90, 'SBP 90 mmHg')
axes[0,0].set_ylim([80, 175])
map_cols = ['TO_MAP','T!_MAP','T2_MAP','T3_MAP','T4_MAP']
plot_hemo(axes[0,1], map_cols, 'MAP (mmHg)', 'B) Mean Arterial Pressure', 65, 'MAP 65 mmHg')
axes[0,1].set_ylim([55, 140])
dia_cols = ['T0_diast','T1_diast','T2_diast','T3_diast','T4_diast']
plot_hemo(axes[1,0], dia_cols, 'DBP (mmHg)', 'C) Diastolic Blood Pressure')
axes[1,0].set_ylim([40, 95])
hr_cols = ['TO_Hf','T!_Hf','T2_Hf','T3_Hf','T4_Hf']
plot_hemo(axes[1,1], hr_cols, 'HR (bpm)', 'D) Heart Rate')
axes[1,1].set_ylim([45, 105])
fig.text(0.5, -0.01, 'Error bars = SEM', ha='center', fontsize=9, style='italic', color='#666')
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, 'Fig3_Hemodynamics.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plot_vars = [
    ('Remifentanil', 'Remifentanil (µg)', 'A) Remifentanil Dose'),
    ('Akrinor', 'Cafedrine-Theodren. (ml)', 'B) Cafedrine-Theodrenaline'),
    ('Noradrenalin', 'Noradrenaline (µg)', 'C) Noradrenaline')
]
for ax, (var, ylabel, title) in zip(axes, plot_vars):
    data_s = sevo[var].dropna()
    data_e = etom[var].dropna()
    bp = ax.boxplot([data_s, data_e], labels=['Sevoflurane', 'Etomidate'],
    ...
    ax.set_title(f'{title}\nP {fmt_p(p)}', fontweight='bold')
    ax.set_ylabel(ylabel)
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, 'Fig4_BoxPlots.png'), dpi=300, bbox_inches='tight')
plt.close()
...
for i, (s, e, h) in enumerate(zip(sevo_hp, etom_hp, hypo_results)):
    ax.text(i, max(s, e, 1) + 2, f'P {fmt_p(h["p"])}', ha='center', fontsize=9)
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, 'Fig5_Hypotension.png'), dpi=300, bbox_inches='tight')
plt.close()
...
ax.plot(fpr1, tpr1, label=f'Unadjusted (AUC = {auc_m1:.3f})', linewidth=2, color='steelblue')
ax.plot(fpr5, tpr5, label=f'Fully Adjusted (AUC = {auc_m5:.3f})', linewidth=2, color='indianred')
ax.plot([0,1],[0,1], 'k--', linewidth=1, alpha=0.5)
ax.set_xlabel('1 − Specificity')
ax.set_ylabel('Sensitivity')
ax.set_title('ROC Curves: Prediction of Vasopressor Requirement', fontweight='bold')
ax.legend(loc='lower right')
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, 'Fig6_ROC.png'), dpi=300, bbox_inches='tight')
plt.close()
...
ax.set_title('Propensity Score Distribution by Treatment Group', fontweight='bold')
ax.legend()
ax.set_xlim([0,1])
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, 'Fig7_PropensityScore.png'), dpi=300, bbox_inches='tight')
plt.close()
print("Saved: Fig7_PropensityScore.png")

# ────────────────────────────────────────────────────────────
# MANUSCRIPT TABLE: LOGISTIC REGRESSION RESULTS
# ────────────────────────────────────────────────────────────
print("\n" + "="*70)
...
for name, or_val, lo, hi, p in results:
    print(f"{name:<35} {or_val:>6.2f} {lo:>6.2f} – {hi:<6.2f}  P {fmt_p(p)}")
